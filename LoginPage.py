...
class Login:
    ...
    def validate(self):
        ...
        if (self.login_type == "Admin"):
            query = "Select * from diya.Admin where AdminName= %s and AdminPassword=%s"
        else:
            query = "Select Uid from diya.User where UserName= %s and UserPassword=%s"
        parameters = (username, pwd)
        #resulttup = DatabaseHelper.get_data(query, parameters)
        ...
        if (result is None):
           ...
           #messagebox.showinfo('Login Success', "Login successfuly completed")
            ...
            id = result.get('Uid')
           # results = DatabaseHelper.get_data(query, parameters)
            print(username)
            print(pwd)
           # Destorying the temporary window created for login
            self.login_window.destroy()
            self.main_page.redirect_to_page(id, self.login_type)
            # self.f.destroy()
